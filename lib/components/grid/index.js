...
function isMobile(coords, start, mobileSize) {
  const size = mobileSize - 1;
  const minX = Math.floor(start.x - size / 2);
  const maxX = Math.floor(start.x + size / 2);
  const minY = Math.floor(start.y - size / 2);
  const maxY = Math.floor(start.y + size / 2);
